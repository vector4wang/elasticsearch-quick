	...
	public static XContentBuilder buildMapping() {
		...
		try {
			mapping = jsonBuilder()
					...
					.field("insertTime4es").startObject().field("type", "date").field("format","yyyy-MM-dd HH:mm:ss||yyyy-MM-dd||epoch_millis").endObject()
					.field("publishDate4es").startObject().field("type", "date").field("format","yyyy-MM-dd HH:mm:ss||yyyy-MM-dd||epoch_millis").endObject()
